"""
Profile MobileNetV2 DualConv variants: params, FLOPs/MACs, model size, latency.

Run from project root:
  python scripts/profile_dualconv_variants.py --input_res 32 --small_input
  python scripts/profile_dualconv_variants.py --input_res 64 --num_classes 200
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Profile MobileNetV2 DualConv variants")
    parser.add_argument(
        "--num_classes",
        type=int,
        default=None,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="DualConv group count G (paper parameter).",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
...
def main() -> None:
    ...
    models = [
        (
            f"MobileNetV2 Baseline (width_mult={args.width_mult})",
            MobileNetV2Baseline(
        ...
        (
            f"MobileNetV2 DualConv-All (G={args.dualconv_groups}, width_mult={args.width_mult})",
            MobileNetV2DualConvAll(
                num_classes=num_classes,
                width_mult=args.width_mult,
                dropout=0.2,
                small_input=args.small_input,
                dualconv_groups=args.dualconv_groups,
            ),
        ),
        (
            f"MobileNetV2 DualConv-B4B10 (G={args.dualconv_groups}, width_mult={args.width_mult})",
            MobileNetV2DualConvB4B10(
                num_classes=num_classes,
                width_mult=args.width_mult,
                dropout=0.2,
        ...
        (
            f"MobileNetV2 DualConv-B4B7 (G={args.dualconv_groups}, width_mult={args.width_mult})",
            MobileNetV2DualConvB4B7(
                num_classes=num_classes,
                width_mult=args.width_mult,
                dropout=0.2,
    ...
    # Baseline reference (for % deltas). Keep it deterministic and consistent with the printed baseline.
